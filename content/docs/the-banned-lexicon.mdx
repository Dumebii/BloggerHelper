...
## Run the Lexicon Against Your Own Writing

The same lists this page describes are what powers Ozigi's free [AI Slop Checker](/slop-checker). Paste in an email, a post, or a draft, and it scores the text for how much of this vocabulary and structure shows up per hundred words, highlights every hit, and explains what each one is doing. It runs entirely in the browser — the word lists ship with the page, so nothing you paste is uploaded or stored.

It's the fastest way to see what the constraint engine is actually enforcing, without generating anything.
